    ...
    for filename in os.listdir(origemValue.get().replace(os.sep,'/')):    
        ...
        if os.path.isfile(file):
            ...
            try:
                ...
                error = error + 1
                messagebox.showerror("Error",f'Erro do arquivo: {filename}')
    Label(fg="green",text=f'Conversão Concluída vom sucesso. {tamanho-error} arquivo(s) convertido(s) e {error} erro(s)', font="arial 10").place(x=50,y=50)
...
def main():
    ...
    # Submit Button
    Button(text="Converter",font=3,width=8,height=1,command=convert).place(x=280,y=170)
    # Exit Button
    Button(text="Sair",font=3,width=8,height=1, command=root.destroy).place(x=160,y=170)
